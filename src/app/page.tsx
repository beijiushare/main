...
export default function HomePage() {
  ...
  useLayoutEffect(() => {
    ...
    const onPhysicsWheel = (e: WheelEvent) => {
      // OptionWheel 内部滚动已 stopPropagation，不会到这里
      e.preventDefault()
      const delta = e.deltaMode === 1 ? e.deltaY * 24 : e.deltaY
      physics.velocity = Math.min(physics.maxVelocity, Math.max(-physics.maxVelocity, physics.velocity + delta * physics.acceleration))
    }
    ...
    // ─── 标题：缩小到左上角 0→180vh ───
    // 注意：GSAP 原生 scrub 驱动，比手动 progress() 映射更平滑
