export function getAcbScrollbarCSS(theme: string, thumbColor: string) {
  const match = thumbColor.match(/[\d.]+(?=\))/)
  const alpha = match ? Math.min(parseFloat(match[0]) + 0.2, 1) : 0.5
  const hoverColor = thumbColor.replace(/[\d.]+(?=\))/, String(alpha))
...
export function getAcbPrismCSS(theme: string) {
  const base = `
